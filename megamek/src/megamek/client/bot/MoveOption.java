...
public class MoveOption extends MovePath {
    ...
    public MoveOption addStep(MoveStepType step_type) {
        ...
        PilotingRollData rollTarget = getEntity().checkRunningWithDamage(current.getMovementType(true, isJumping()));
    ...
    public int getMovementheatBuildup() {
        ...
        int move = 0;
        switch (last.getMovementType(true, isJumping())) {
        case MOVE_WALK:
        case MOVE_VTOL_WALK:
            move = 1;
            break;
    ...
    public boolean changeToPhysical() {
        ...
        if ((last == null)
                || (last.getMovementType(true, isJumping()) == EntityMovementType.MOVE_ILLEGAL)) {
            return false;
        }
